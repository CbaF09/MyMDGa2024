...
namespace tnl {
	...
	//	public:
	//		// シーケンス機能の宣言
	//		SEQUENCE(Example, &Example::seqCallFunctionA); // 初動で seqCallFunctionA が実行されます
	// 
	//		// 毎フレーム実行される更新処理
	...
	//		if( seq_.getProgressTime() > 5.0f ){
	//			seq_.change( &Example::seqCallFunctionB ) ;
	//		}
	// 
	//		return true ;
	// }
	...
	//		SEQ_CO_YIELD_RETURN_FRAME(10, delta_time, [&] {
	// 
	//			// 最初の１フレームのみ真になる
	//			if ( SEQ_CO_YIELD_FRAME_IS_START ) {
	//			}
	// 
	//		});
	// 
	...
	//		SEQ_CO_YIELD_RETURN_TIME(1.0f, delta_time, [&] {
	// 
	//			// 最初の１フレームのみ真になる
	//			if ( SEQ_CO_YIELD_FRAME_IS_START ) {
	//			}
	...
	//		SEQ_CO_YIELD_RETURN_FRAME(60, delta_time, [&] {
	// 
	//			// このコルーチンの経過フレーム数が 30 になったら真
	//			if( SEQ_CO_PROG_FRAME == 30 ){
	//				// このコルーチンを破棄する
	//				SEQ_CO_BREAK ;
	//			}
	// 
	...
	//		seq_.invoke( &Example::calledFromInvokeA, 1.0f);
	// 
	//		// seqCallFunctionC を 3秒後に 0.5秒間隔で実行
	//		seq_.invokeRepeating( &Example::calledFromInvokeB, 3.0f, 0.5f);
	// 
	//		// 指定した invoke を全てキャンセル
	//		seq_.cancelInvoke( &Example::calledFromInvokeB ) ;
	// }
	// 
	...
	public:
		...
		// arg1.. 包含クラス名
		// arg2.. 初期シーケンス
#define SEQUENCE( class_name, start_func )	tnl::Sequence<class_name> seq_ = tnl::Sequence<class_name>(this, start_func);

// コルーチンシーケンス終了処理
// tips.. コルーチン使用メソッドの最後に必ずコールすること
#define SEQ_CO_END								seq_._co_reset_call_count_(); return true ;

// コルーチン実行フレーム数の取得
#define SEQ_CO_PROG_FRAME						seq_._co_get_prog_frame_()

// コルーチン実行時間の取得
#define SEQ_CO_PROG_TIME						seq_._co_get_prog_time_()

// 当該コルーチンの破棄
#define SEQ_CO_BREAK							{ seq_._co_break_() ; return ; }
...
#define SEQ_CO_YIELD_RETURN_FRAME( lim_frame, delta_time, logic )		if (seq_._co_yield_by_frame_( lim_frame, delta_time, logic ) ) return true ;
...
// ....... フレーム数カウントは int32_t を超えると 0 にリセットされることに注意
#define SEQ_CO_YIELD_RETURN_TIME( lim_time, delta_time, logic )		if (seq_._co_yield_by_time_( lim_time, delta_time, logic ) ) return true ;


// コルーチン内部(SEQ_CO_YIELD_RETURN_FRAME) の最初のフレームなら true
#define SEQ_CO_YIELD_FRAME_IS_START		( 0 == seq_._co_get_prog_frame_() )
// コルーチン内部(SEQ_CO_YIELD_RETURN_FRAME) の最後のフレームなら true
#define SEQ_CO_YIELD_FRAME_IS_END		( seq_._co_get_limit_frame_() == (seq_._co_get_prog_frame_()+1) )

// コルーチン内部(SEQ_CO_YIELD_RETURN_TIME) の最初のフレームなら true
#define SEQ_CO_YIELD_TIME_IS_START		( 0 == seq_._co_get_prog_frame_() )
// コルーチン内部(SEQ_CO_YIELD_RETURN_TIME) の最初のフレームなら true
#define SEQ_CO_YIELD_TIME_IS_END			( seq_._co_get_limit_time_() <= seq_._co_get_prog_time_() )
